...
function Detail() {
    // 우리가 생각할 때, 이미 App에서 Routing 통해 /:id가 들어와야만, Detail 뜨지 않나?
    ...
    // 배열은 [] 빈배열을 해도 상관 없지만, 왜냐하면 얘는 그 형태의 요소가 없을 뿐
    ...
    useEffect(() => {
        ...
        // 그래서 fetch를 실행시키 전에, if를 통해 id값이 없다면 뒤를 실행하지 말고 이 함수를 종료해라라고 해줘야 함
        if (!id) return;
